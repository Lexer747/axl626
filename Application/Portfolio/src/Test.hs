...
problemDef :: String
problemDef = concat $ replicate 1 "fNortOfe"

genomesize :: Int
genomesize = boolGenomeLengthForProblem problemDef
...
main :: IO()
main = do
  finalPop <- geneticAlgorithm problemDef
  let winner = takeGenome . head . bestFirst Maximizing $ finalPop
  putStrLn $ showGenome problemDef winner
  return ()
