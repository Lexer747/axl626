...
\subsection{Results}
    ...
    set of portfolio's produced by the GA, only 3 are chosen. It could have been worth
    it to test all of the portfolio's produced by the GA. But since you can only
    chose a single portfolio from the set, its realistic unfeasible to expect to
    test all the portfolio's. Since a human should make a choice about how much
    ...
    portfolio's to test, representing 3 different peoples decision making; a risk
    ...
    These 3 portfolio's were then carried forward and tested. From here on out
    these 3 portfolio's are referred to as Highest \(G\) (the first element),
    ...
    the portfolio's chosen by the naive spread, risk averse, risk neutral, risk
    ...
    The variance of the portfolio's produced by the GA are not only lower, meaning that
    ...
    had a lower variance and hence are less risky. The lowest risk portfolio's
    had a 10x improvement in risk compared to the median portfolios, and 20x
    compared to the highest \(G\) portfolios.

    For completeness I also plotted the actual portfolio's created by the GA, as this
    would indicate if the GA was over fitting or finding local maximums.
...
\subsection{A Comparison Of Portfolios Table}
    \begin{table}[H]\label{apd:AComparisonOfPortfoliosTable}
        \begin{tabular}
...
\subsection{Mean and Variance of Portfolio Types}
    \begin{table}[H]\label{apd:MeanAndVarianceOfPortfolioTypes}
        \begin{tabular}
...
\subsection{Highest \(G\) Portfolio Distributions}
    \begin{figure}[H] % fig:HighestGPortfolioDistributions
        % \centering
        \includegraphics[width=\textwidth]{HighestGPortfolioDistributions}
        \caption{-}
            \label{fig:HighestGPortfolioDistributions}
    \end{figure}

\subsection{Median \(G/R\) Portfolio Distributions}
    \begin{figure}[H] % fig:MedianGRPortfolioDistributions
        % \centering
        \includegraphics[width=\textwidth]{MedianGRPortfolioDistributions}
        \caption{-}
            \label{fig:LowestRPortolioDistributions}
    \end{figure}

\subsection{Lowest \(R\) Portfolio Distributions}
    \begin{figure}[H] % fig:LowestRPortolioDistributions
        % \centering
