...
\subsection{NSGA-II vs Other Multi-Objective Algorithms}
    ...
    This is denoted hence forth as \(x \prec y\), and among a set of solutions \(P\)
    the non dominated sub set \(P'\) are those that are not dominated by any member of
    set \(P\). To follow on from this the non-dominate set of the multi-objective search
...
\subsection{Results}
    ...
    \begin{figure}[H] % fig:GainOfthePortfolio-OverDifferentWindows
        % \centering
        \includegraphics[width=\textwidth]{GainOfthePortfolio-OverDifferentWindows}
        \caption{Gain of GA vs Naive vs UK interest}
            \label{fig:GainOfthePortfolio-OverDifferentWindows}
    \end{figure}

    \begin{figure}[H] % fig:GeneralStockDistributionCombinedOverAllWindowsTested
        % \centering
    ...
    \begin{figure}[H] % fig:HowTheInduvidualPortfolioWasDistributedOverEachWindow
        % \centering
