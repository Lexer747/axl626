...
& \Big( m = \prod^{n}_{i=1} x_i (x \text{ is 2 for a bet, win and lose. Or 1 for a trade, the value gained}) \Big) &&\\
...
simplicity we assume that the correlation coefficients are zero. I.e. each coin is independent.
And modeling the risk more simply than a stock.

This means we need to find 3 f values \(f_1, f_2, f_3\) to result in the greatest growth. Or 
the  3 f's which result in the greatest \(G\).
